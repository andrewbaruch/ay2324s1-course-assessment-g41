...
export const CodeEditor = () => {
  ...
  const { sharedValue: sharedLanguage }: { sharedValue: { label: string, value: string } } = useSharedDocument({
    sharedKey: "language", valueToShare: language, document
  })
  ...
  return (
    <Stack>
      <Box w="fit-content" minW="300px">
        <Select value={sharedLanguage} options={supportedLanguages} onChange={(newValue) => changeLanguage(newValue as { label: string, value: string })} />
      </Box>
